...
#if !defined(ZEN_ENABLE_EVM)
    BlockSymbol = Ctx.getOrCreateSymbol(Twine(Prefix) + "BB" +
                                        "_" + Twine(getNumber()));
#else
    std::string BlockComment = getSourceName().empty() ? std::to_string(getNumber())
                                               : getSourceName() + "_" +
                                               std::to_string(getSourceOffset());
    BlockSymbol = Ctx.getOrCreateSymbol("EVMBB" +
